...
def get_products_list():
    ...
    #products = db.session.execute(statement=stmt).scalars()
    products: Sequence[Product] = db.session.scalars(statement=stmt)
...
def create_product():
    ...
    try:
        ...
        flash(f"Product {product.name!r} already exists!", category="warning")
...
def get_prod_by_id(product_id: int) -> Product:
...
def get_product_by_id(product_id: int):
    ...
    return render_template(
        ...
        product=get_prod_by_id(product_id=product_id),
    )

@products_app.route("<int:product_id>/confirm_delete/",
                    methods=["GET", "POST"],
                    endpoint="confirm_delete")
def confirm_delete_product(product_id: int):
    product = get_prod_by_id(product_id=product_id)
    if request.method == "GET":
        return render_template("products/confirm-delete.html",
                               product = product)
    name = product.name
